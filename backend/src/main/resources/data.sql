...
-- ------------------------------------------------------------------
-- 7. Resync AUTO_INCREMENT counters so future JPA-generated inserts
--    continue after the max seeded id in each table (MySQL syntax -
--    ALTER TABLE ... AUTO_INCREMENT = N does not accept a subquery,
--    so the next-id values below are hardcoded from the inserts above).
-- ------------------------------------------------------------------
ALTER TABLE station        AUTO_INCREMENT = 6;   -- 5 rows seeded (ids 1-5)
ALTER TABLE track_section  AUTO_INCREMENT = 5;   -- 4 rows seeded (ids 1-4)
ALTER TABLE platform       AUTO_INCREMENT = 14;  -- 13 rows seeded (ids 1-13)
ALTER TABLE loop_line      AUTO_INCREMENT = 4;   -- 3 rows seeded (ids 1-3)
ALTER TABLE train          AUTO_INCREMENT = 11;  -- 10 rows seeded (ids 1-10)
ALTER TABLE train_schedule AUTO_INCREMENT = 44;  -- 43 rows seeded (ids 1-43)
